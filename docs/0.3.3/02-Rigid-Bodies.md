...
You can override several methods in your `VxRigidBody` class to add custom server-side logic at different points in its lifecycle.

*   **`onBodyAdded(VxPhysicsWorld world)`**: Called once, right after the Jolt body has been created and added to the simulation. This is the ideal place to create constraints that involve this body.
*   **`onBodyRemoved(VxPhysicsWorld world, VxRemovalReason reason)`**: Called just before the Jolt body is destroyed. Use this for cleanup logic.
*   **`physicsTick(VxPhysicsWorld world)`**: Called every single physics step (typically 60 times per second) on the **physics thread**. Use this for high-frequency logic that needs to be tightly synchronized with the simulation (e.g., applying custom forces).
*   **`gameTick(ServerLevel level)`**: Called every server tick (typically 20 times per second) on the **main server thread**. Use this for standard game logic, such as interacting with the Minecraft world, entities, or players.